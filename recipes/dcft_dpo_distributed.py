...
class DCFTDPODistributed(FTRecipeInterface):
    """
    DCFT DPO distributed recipe.
    """
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        self.precompute_reference_logprobs = cfg.get("precompute_reference_logprobs", False)
        ...
        if self.precompute_reference_logprobs:
            if self._is_rank_zero:
                log.info("Will precompute reference logprobs in the data setup step.\n"
                         "Creating a reference model is unnecessary when this is enabled.")
        else:
            ...
            if self._is_rank_zero:
                log.info("Reference model is initialized.")
                
        self._sampler, self._dataloader = self._setup_data(
            cfg_dataset=cfg.dataset,
            shuffle=cfg.shuffle,
            batch_size=cfg.batch_size,
            precompute_batch_size=cfg.precompute_batch_size if self.precompute_reference_logprobs else None,
        )
    ...
    def _setup_data(
        ...
        if self.precompute_reference_logprobs:
            precompute_sampler = DistributedSampler(
                ds, num_replicas=world_size, rank=rank, shuffle=False, seed=0
            )
            precompute_dataloader = DataLoader(
                dataset=ds,
                batch_size=precompute_batch_size,
                sampler=precompute_sampler,
                collate_fn=partial(
            ...
            self._model.eval()
            reference_chosen_log_probs_accum_list = []
            reference_rejected_log_probs_accum_list = []
            for batch in tqdm(precompute_dataloader, desc="Precomputing reference logprobs" if self._is_rank_zero else ""):
                with torch.no_grad():
                    (
                ...
                reference_chosen_log_probs_list = [torch.zeros_like(reference_chosen_log_probs) for _ in range(world_size)]
                reference_rejected_log_probs_list = [torch.zeros_like(reference_rejected_log_probs) for _ in range(world_size)]
                    
                if self._is_rank_zero:
                    log.info(f"Reference logprobs: {reference_chosen_log_probs.shape}, {reference_rejected_log_probs.shape}")
                
                # gather from all GPUs
                torch.distributed.all_gather(reference_chosen_log_probs_list, reference_chosen_log_probs)
                torch.distributed.all_gather(reference_rejected_log_probs_list, reference_rejected_log_probs)
                
                if self._is_rank_zero:
                    reference_chosen_log_probs_list = torch.cat(reference_chosen_log_probs_list).cpu()
                    reference_rejected_log_probs_list = torch.cat(reference_rejected_log_probs_list).cpu()
                    
                    log.info(f"Reference logprobs: {reference_chosen_log_probs_list.shape}, {reference_rejected_log_probs_list.shape}")
                    
                    reference_chosen_log_probs_accum_list.append(reference_chosen_log_probs_list)
                    reference_rejected_log_probs_accum_list.append(reference_rejected_log_probs_list)
            ...
            if self._is_rank_zero:
                reference_chosen_log_probs = torch.cat(reference_chosen_log_probs_accum_list).float().numpy()
                reference_rejected_log_probs = torch.cat(reference_rejected_log_probs_accum_list).float().numpy()
                
                ds = ds.add_column("reference_chosen_log_probs", reference_chosen_log_probs)
                ds = ds.add_column("reference_rejected_log_probs", reference_rejected_log_probs)
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                # if precompute_reference_logprobs is True, 
                # batch[2] and batch[3] are reference_chosen_log_probs 
                # and reference_rejected_log_probs respectively
                ...
                ) = self.concatenated_forward(self._model, (batch[0], batch[1]))
                ...
                if len(batch) == 4 and self.precompute_reference_logprobs:
                    reference_chosen_log_probs = batch[2]
                    reference_rejected_log_probs = batch[3]
                else:
                    with torch.no_grad():
                        (
                            reference_chosen_log_probs,
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    config.log_config(recipe_name="DCFTDPODistributed", cfg=cfg)

    recipe = DCFTDPODistributed(cfg=cfg)
